...
corr_names <- unique(unlist(visit_with_cluster$sewershed_analysis_name))
corr_list <- list()
for (i in seq_along(corr_names)){
  corr_list[[i]] <- visit_with_cluster %>% filter(sewershed_analysis_name == corr_names[[i]])
...
flow_corr_daily <- bind_rows(corr_list) %>%
  select(date, sewershed_analysis_name, daily)

# merge with mobile device data
flow_mobile_corr_daily <- merge(flow_corr_daily, visit_with_cluster,
  ...
  drop_na(daily) %>%
  #select(date, sewershed_analysis_name, sewershed_result_name, day_of_week, month, season, year, total_devices_daily_log_norm, flow_rate_log_norm, cluster) %>%
  arrange(sewershed_analysis_name)

# save as Rda
save(flow_mobile_corr_daily, file = "DataProcessed/flow_mobile_corr_daily.Rda")

flow_mobile_corr_monthly <- flow_mobile_corr_daily %>%
  group_by(sewershed_analysis_name) %>%
  select(date, sewershed_analysis_name, cluster, total_devices_monthly_log_norm, flow_rate_log_norm) %>%
  fill(flow_rate_log_norm, .direction = "down") %>%
  drop_na()

flow_mobile_corr <- flow_mobile_corr_monthly %>%
  #drop_na(flow_rate_log_norm) %>%
  group_by(cluster) %>%
  mutate(corr = cor(total_devices_monthly_log_norm, flow_rate_log_norm, method = c("spearman")))

# scatterplots with correlation coefficients
scatter_high <- ggplot(data = flow_mobile_corr %>% filter(cluster == "High-Variation")) +
  geom_point(aes(x = total_devices_monthly_log_norm, y = flow_rate_log_norm)) +
  labs(x = "log(Daily Device Count), Year-Normalized", y = "log(Flow Rate), Year-Normalized") +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        plot.margin = margin(15, 20, 10, 0, "pt")); scatter_high

scatter_low <- ggplot(data = flow_mobile_corr %>% filter(cluster == "Low-Variation")) +
  geom_point(aes(x = total_devices_monthly_log_norm, y = flow_rate_log_norm)) +
  labs(x = "log(Daily Device Count), Year-Normalized", y = "log(Flow Rate), Year-Normalized") +
  ggtitle("Low-Variation Sewersheds (n = 15)") +
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(angle = 90, hjust = 0.95, vjust = 0.4),
        axis.title.y = element_text(margin = margin(l = 10, r = 5)),
        plot.margin = margin(15, 20, 10, 0, "pt")); scatter_low


  #scale_x_continuous(labels = scales::comma, expand = expansion(mult = c(0.1, 0.5))) +
  #scale_y_continuous(expand = expansion(mult = c(0.1, 0.5))) +
  #geom_text(data = spearman_df2, aes(x = total_devices_daily_log_norm*1.2,
                                     #y = flow_rate_log_norm*1.5,
...
flow_mobile_corr_complete <- flow_mobile_corr_daily %>%
  ...
  mutate(corr = cor(total_devices_daily_log_norm, flow_rate_log_norm, method = c("spearman")))
...
spearman_df1 <- flow_mobile_corr_complete %>%
  select(corr, total_devices_daily_log_norm) %>%
  arrange(desc(total_devices_daily_log_norm)) %>%
  slice(1)

spearman_df2 <- flow_mobile_corr_complete %>%
  select(corr, flow_rate_log_norm) %>%
  arrange(desc(flow_rate_log_norm)) %>%
  slice(1) %>%
...
scatter <- ggplot(data = flow_mobile_corr_complete) +
  geom_point(aes(x = total_devices_daily_log_norm, y = flow_rate_log_norm)) +
  labs(x = "log(Daily Device Count), Year-Normalized", y = "log(Flow Rate), Year-Normalized") +
